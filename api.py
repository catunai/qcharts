...
@router.get("/report_data_melted", response_model=List[serializers.AttemptDetails])
def report_data_melted(
    ...
    attempt_details_stmt = select(models.AttemptDetails).where(
        and_(
            models.AttemptDetails.quote_channel == channel,
            models.AttemptDetails.product == product,
            models.AttemptDetails.date_type == date_type,
        )
    )

    return request.state.db.scalars(attempt_details_stmt).all()
